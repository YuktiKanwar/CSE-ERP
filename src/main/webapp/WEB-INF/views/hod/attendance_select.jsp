...
<div class="container">
		...
		<table class="tg row-border hover" id="timetable_list">
			...
			<c:forEach items="${listTimeTables}" var="timetable">
				<tr>
					...
					<td>
						<a onclick="markAttendance(${timetable.id},'${facultyName}')" >Mark</a>
					</td>
				</tr>
		</c:forEach>
		</table>
	...
	var dateString = "";
	function markAttendance(timetableid, facultyName){
		dateString = dateString.replace('/','-');
		dateString = dateString.replace('/','-');
		window.location.href = "http://localhost:8080/ERPApp/Faculty/attendance/mark/" + timetableid + "/" + dateString + "/" + facultyName; 
	}
